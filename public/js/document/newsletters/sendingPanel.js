...
pimcore.registerNS("pimcore.bundle.newsletter.document.newsletters.sendingPanel");
/**
 * @private
 */
pimcore.bundle.newsletter.document.newsletters.sendingPanel = Class.create({
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            ...
            for(var i = 0; i < adapterNames.length; i++) {
                ...
                if (requiredNamespace && (typeof pimcore.bundle[requiredNamespace] === 'undefined' || typeof pimcore.bundle[requiredNamespace]['startup'] === 'undefined')) {
                    continue;
                }
            ...
            this.sourceAdapterSelection = Ext.create('Ext.form.ComboBox', {
                ...
                listeners: {
                    select: function(combo, record, eOpts) {
                        ...
                        this.currentSourceAdapter = new pimcore.bundle.newsletter.document.newsletters.addressSourceAdapters[record.data.key](this.document);
            ...
            this.statusUpdateBox = Ext.create('Ext.Panel', {
                ...
                items: [this.progressBar, {
                    ...
                    handler: function() {
                        Ext.Ajax.request({
                            url: Routing.generate('pimcore_bundle_newsletter_document_newsletter_stopsend'),
                            method: "POST",
    ...
    send: function() {
        ...
        Ext.Ajax.request({
            url: Routing.generate('pimcore_bundle_newsletter_document_newsletter_calculate'),
            method: "post",
            params: params,
            success: function(response) {
                ...
                if(res.success) {
                    ...
                    Ext.MessageBox.confirm(t("are_you_sure"), msg, function (buttonValue) {
                        ...
                        if (buttonValue == "yes") {
                            Ext.Ajax.request({
                                url: Routing.generate('pimcore_bundle_newsletter_document_newsletter_send'),
                                method: "post",
                                params: params,
                                success: function (response) {
    ...
    sendTest: function() {
        ...
        Ext.Ajax.request({
            url: Routing.generate('pimcore_bundle_newsletter_document_newsletter_sendtest'),
            method: "post",
            params: params,
            success: function(response) {
                var res = Ext.decode(response.responseText);
    ...
    checkForActiveSendingProcess: function() {
        Ext.Ajax.request({
            url: Routing.generate('pimcore_bundle_newsletter_document_newsletter_getsendstatus'),
            params: {id: this.document.id},
            success: function(response) {
